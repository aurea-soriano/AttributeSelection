 ...
 * @author aurea
 */
public class JointEntropy implements Measure {
    ...
    public Object[][] calculateRanking(ArrayList<String> attributes, ArrayList<String> selectedAttributes, AbstractMatrix pointsMatrix) {
        ...
        for (int i = 0; i < valuesCount; i++) {
            ...
            double value = Utils.jointEntropy(values, values);
    ...
    public Object[][] calculateMatrix(ArrayList<String> attributes, ArrayList<String> selectedAttributes, AbstractMatrix pointsMatrix, Integer numberThreads) {
        ...
        for (int i = 0; i < valuesCount - 1; i++) {
            for (int j = 1; j < valuesCount; j++) {
                ...
                try {
                    if (i != j - 1) {
                        double value = Utils.jointEntropy(values1, values2);
                        DecimalFormatSymbols dfs = new DecimalFormatSymbols();
                        dfs.setDecimalSeparator('.');
                        DecimalFormat df = new DecimalFormat("0.000", dfs);
                        String valueString = df.format(value);
        ...
        for (int i = 1; i <= numberThreads; i++) {
            ...
            listThreads.add(new JoinEntropyThread(beginPoint - 1, endPoint - 1, i));
    ...
    public void calculateJoinEntropyThreads(Integer beginPoint, Integer endPoint, Integer numberThread) {
    ...
    class JoinEntropyThread extends Thread {

        Integer beginPoint;
        Integer endPoint;
        Integer numberThread;

        public JoinEntropyThread(Integer beginPoint, Integer endPoint, Integer numberThread) {
        ...
        @Override
        public void run() {
            calculateJoinEntropyThreads(this.beginPoint, this.endPoint, this.numberThread);
        }
    }
}
