...
public class Covariance implements Measure {
    ...
    public Object[][] calculateRanking(ArrayList<String> attributes, ArrayList<String> selectedAttributes, AbstractMatrix pointsMatrix) {
        ...
        for (int i = 0; i < valuesCount; i++) {
            ...
            double value = Utils.covariance(values, values);
    ...
    public Object[][] calculateMatrix(ArrayList<String> attributes, ArrayList<String> selectedAttributes, AbstractMatrix pointsMatrix, Integer numberThreads) {
        ...
        /*  for (int i = 0; i < valuesCount - 1; i++) {
         ...
         double value = Utils.covariance(values1, values2);
         DecimalFormatSymbols dfs = new DecimalFormatSymbols();
         dfs.setDecimalSeparator('.');
         DecimalFormat df = new DecimalFormat("0.000", dfs);
         String valueString = df.format(value);
        ...
        for (int i = 1; i <= numberThreads; i++) {
            ...
            listThreads.add(new CovarianceThread(beginPoint - 1, endPoint - 1, i));
    ...
    public void calculateCovarianceThreads(Integer beginPoint, Integer endPoint, Integer numberThread) {
        for (int i = beginPoint; i <= endPoint; i++) {
            ...
            for (int j = i + 1; j < (valuesCount - 1); j++) {
                ...
                try {
                    ...
                    if (i != j) {
                        double value = Utils.covariance(values1, values2, mean1);
                        DecimalFormatSymbols dfs = new DecimalFormatSymbols();
                        dfs.setDecimalSeparator('.');
                        DecimalFormat df = new DecimalFormat("0.000", dfs);
                        String valueString = df.format(value);
    ...
    class CovarianceThread extends Thread {

        Integer beginPoint;
        Integer endPoint;
        Integer numberThread;

        public CovarianceThread(Integer beginPoint, Integer endPoint, Integer numberThread) {
        ...
        @Override
        public void run() {
            calculateCovarianceThreads(this.beginPoint, this.endPoint, this.numberThread);
        }
    }
}
