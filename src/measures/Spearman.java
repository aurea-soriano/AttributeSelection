 ...
 * @author aurea
 */
public class Spearman implements Measure {
    ...
    public Object[][] calculateRanking(ArrayList<String> attributes, ArrayList<String> selectedAttributes, AbstractMatrix pointsMatrix) {
        ...
        for (int i = 0; i < valuesCount; i++) {
            ...
            double value = Utils.spearman(values, values);
    ...
    public Object[][] calculateMatrix(ArrayList<String> attributes, ArrayList<String> selectedAttributes, AbstractMatrix pointsMatrix, Integer numberThreads) {
        ...
        /* for (int i = 0; i < valuesCount - 1; i++) {
         ...
         try {
         if (i != j - 1) {
         double value = Utils.spearman(values1, values2);
         DecimalFormatSymbols dfs = new DecimalFormatSymbols();
         dfs.setDecimalSeparator('.');
         DecimalFormat df = new DecimalFormat("0.000", dfs);
         String valueString = df.format(value);
        ...
        for (int i = 1; i <= numberThreads; i++) {
            ...
            listThreads.add(new SpearmanThread(beginPoint - 1, endPoint - 1, i));
    ...
    public void calculateSpearmanThreads(Integer beginPoint, Integer endPoint, Integer numberThread) {
    ...
    class SpearmanThread extends Thread {

        Integer beginPoint;
        Integer endPoint;
        Integer numberThread;

        public SpearmanThread(Integer beginPoint, Integer endPoint, Integer numberThread) {
        ...
        @Override
        public void run() {
            calculateSpearmanThreads(this.beginPoint, this.endPoint, this.numberThread);
        }
    }
}
