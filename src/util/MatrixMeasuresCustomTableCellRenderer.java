 ...
 * @author aurea
 */
public class MatrixMeasuresCustomTableCellRenderer extends DefaultTableCellRenderer {
    ...
    public MatrixMeasuresCustomTableCellRenderer(String colorScaleString, Double max, Double min) {
        try {
        ...
        if (table.isCellSelected(row, column) && column != 0) {
            ...
            if (column == 0) {
                ...
                try {
                    ...
                    int position;
                    if (maxValue > 1.0) {
                        position = (int) Math.round(Utils.minmax(maxValue, minValue, 254.0, 0.0, value1));
                    } else {
                        position = (int) Math.round(Utils.minmax(1.0, 0.0, 254.0, 0.0, value1));
                    }
                    if (position > 255) {
                        position = 255;
                    } else {
